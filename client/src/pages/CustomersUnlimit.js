...
import { UNLIMIT } from '../consts/plans'
...
const CustomersToday = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      const res = await fetch(`http://localhost:5000/customers/${UNLIMIT}`);
  ...
  if (isLoading) {
    ...
    return (
      <div className="CustomersToday">
        <h3 className="text-center mb-3">Все клиенты c тарифом "Безлимит"</h3>
        <Row>
